package com.edu.run.presentation.active_run.service
...
class ActiveRunService: Service() {
    ...
    private val baseNotification by lazy{
        NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            ...
            .setContentTitle(getString(com.edu.run.presentation.R.string.active_run))
    }
    ...
    private fun start(activityClass: Class<*>){
        if(!isServiceActive){
            ...
            val activityIntent = Intent(applicationContext, activityClass).apply {
                data = "runcounter://active_run".toUri()
    ...
    private fun createNotificationChannel() {
        if(Build.VERSION.SDK_INT >= 26){
            val channel = NotificationChannel(
                CHANNEL_ID,
                getString(com.edu.run.presentation.R.string.active_run),
    ...
    companion object{
        ...
        fun createStartIntent(context: Context, activityClass: Class<*>) = Intent(context,
            ActiveRunService::class.java).apply {
                action = ACTION_START
            putExtra(EXTRA_ACTIVITY_CLASS,activityClass.name)
        }
        fun createStopIntent(context: Context) = Intent(context, ActiveRunService::class.java).apply {
            action = ACTION_STOP
        }
    }
